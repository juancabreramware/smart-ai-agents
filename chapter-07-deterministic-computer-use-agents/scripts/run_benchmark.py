...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run the Chapter 7 150-request benchmark across Baseline, Smart, and Naive architectures.")
    ap.add_argument("--planner", choices=["mock", "openai"], default="mock")
    ap.add_argument("--workload", type=Path, default=ROOT / "workloads" / "benchmark_v1.0.jsonl")
    ap.add_argument("--output", type=Path, default=ROOT / "evidence" / "mock-validation")
    ap.add_argument("--headed", action="store_true")
    ap.add_argument("--executor", choices=["playwright", "simulated"], default="playwright")
    args = ap.parse_args()
    requests = load_jsonl(args.workload)
    if len(requests) != 150:
        raise SystemExit(f"Canonical benchmark workload must contain exactly 150 requests; got {len(requests)}")
