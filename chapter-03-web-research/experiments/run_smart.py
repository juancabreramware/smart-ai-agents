...
def main() -> None:
    ...
    result, ledger = SmartPricingAgent(provider_from_name(args.provider)).run(
    ...
    print(
        f"\npath={ledger.path.value} model_calls={ledger.model.calls} "
        f"capability={ledger.capability_id}@v{ledger.capability_version} "
        f"validation={ledger.validation_passed} cost=${ledger.total_cost_usd:.6f}"
    )
